...
#First part
 surveys_wide <- surveys %>% 
  filter(!is.na(hindfoot_length)) %>% #remove the NAs 
...
#Using ifelse()
surveys$weightcat <- ifelse(surveys$weight <= 20, "small", ifelse(surveys$weight >20 & surveys$weight <48,"medium", "big")

head(surveys)
str(surveys$weightcat)

#using case_when function

surveys %>% 
  mutate(weightcat1 = case_when(
